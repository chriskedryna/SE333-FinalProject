...
9. You will run a SpotBugs analysis. SpotBugs should be version 4.9.8.2 in the pom.xml. You will need to do `cd SE333-Demo/projectAnalyzed/spring-petclinic/spring-petclinic`. Run SpotBugs analysis by running `mvn spotbugs:spotbugs`. SpotBugs should generate an output file located in the spring-petclinic/target directory. This file will be called `spotbugsXml.xml`. Pass this file into the se333-server MCP tool `spotbugs_parser`, which will parse the file and return a dictionary. If critical/high issues are found, address the bugs and rerun SpotBugs. If issues persist due to tool limitations, proceed.
